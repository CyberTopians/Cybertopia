...
var LegendScale = class extends LinearScale {
  ...
  draw(chartArea) {
    ...
    const bak = this.options.position;
    this.options.position = this.options.legend.align;
    super.draw(__spreadProps(__spreadValues({}, chartArea), { bottom: this.height + 10, right: this.width }));
    this.options.position = bak;
...
var SizeScale = class extends LegendScale {
  ...
  _drawIndicator() {
    ...
    values.forEach((v, i) => {
      ...
      const renderOptions = __spreadProps(__spreadValues({
        pointStyle: "circle",
        borderWidth: 0
      }, this._model || {}), {
        radius
...
var GeoFeature = class extends Element {
  ...
  getCenterPoint() {
    ...
    this.cache = __spreadProps(__spreadValues({}, this.cache || {}), { center });
    return center;
  }
  getBounds() {
    ...
    this.cache = __spreadProps(__spreadValues({}, this.cache || {}), { bounds });
    return bounds;
  }
  _drawInCache(doc) {
    ...
    if (ctx) {
      ...
      this._drawImpl(ctx);
      ctx.restore();
      this.cache = __spreadProps(__spreadValues({}, this.cache || {}), { canvas, canvasKey: this._optionsToKey() });
    }
  }
  _optionsToKey() {
    const { options } = this;
...
GeoFeature.defaults = __spreadProps(__spreadValues({}, BarElement.defaults), {
  outlineBackgroundColor: null,
  outlineBorderWidth: 0,
  graticuleBorderColor: "#CCCCCC",
  graticuleBorderWidth: 0
});
GeoFeature.defaultRoutes = __spreadValues({
...
function patchDatasetElementOptions(options) {
  const r = __spreadValues({}, options);
